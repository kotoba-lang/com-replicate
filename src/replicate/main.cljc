(ns replicate.main
  "Kotodama WASM entrypoint for the Replicate clean-room actor (L4) — Clojure port.
  ...
  facts (`replicate.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "replicate")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"replicate.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "replicate-compat" :tier tier :entities entities} 200])
...
(defn start! [] :replicate-compat/ready)
